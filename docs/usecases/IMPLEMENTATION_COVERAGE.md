...
| Caller authority / confidence override         | Removed / Fail-closed                 | `app/api/demo/answer/handler.ts`; `app/backend/assistant/logistics/composition.ts`; `tests/answer-route.test.ts`                                        | Caller cannot supply confidence, role, guild, or provider keys; derived strictly from server session membership.                           |
...
| Tool execution boundary                        | Implemented (read-only allowlist)     | `app/backend/assistant/logistics/tools.ts`; `tests/logistics-assistant.test.ts`                                                                         | Mutation tools remain disabled for learner answer path.                                                                                    |
...
8. **Deterministic production composition:**
   - The production route composition uses deterministic notice retrieval directly against user-scoped Supabase client. The optional draft-provider port and read-only tool executor are exercised via fakes in tests, while live LLM generation remains disabled pending separate review.
