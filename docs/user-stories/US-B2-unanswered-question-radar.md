...
- **Then** Hệ thống xuất bản một bản tin có cấu trúc chuẩn vào kênh `#ta-radar` gồm 3 phần:
  1. _Thống kê tổng quan:_ Tổng số câu hỏi trong ngày, số câu đã giải quyết, số câu còn tồn đọng (>2h và >4h).
  2. _Danh sách câu hỏi cần xử lý gấp:_ Liệt kê từng câu hỏi tồn đọng kèm tóm tắt 1 câu, tên học viên và liên kết xem tin nhắn.
  3. _Top 3 chủ đề nóng nhất:_ Tổng hợp các vấn đề học viên hỏi nhiều nhất trong ngày để TA cân nhắc đăng thông báo chung.
...
- **Then** Hệ thống gửi một phản hồi nhẹ nhàng ngay dưới thread công khai của học viên, gợi ý 1-2 tài liệu/hướng đi ngắn gọn từ kho tài liệu chính thức: _"Nếu bạn đang gặp lỗi này, bạn có thể tham khảo mục Cài đặt môi trường tại [Link tài liệu]. Mình cũng đã báo anh/chị TA hỗ trợ bạn nhé!"_,
...
### AC5: Có phản hồi không đồng nghĩa đã giải quyết

- **Given** Một câu hỏi đang nằm trong danh sách cảnh báo tồn đọng của Radar,
- **When** Một trợ giảng hoặc học viên khác gửi reply trong thread,
- **Then** Hệ thống cập nhật câu hỏi sang `ANSWERED` nhưng vẫn giữ trên radar,
- **And** chỉ một Lab Coach được xác thực mới có thể chuyển câu hỏi sang `RESOLVED` bằng thao tác có khóa phiên bản,
- **And** Radar chỉ ngừng theo dõi sau thao tác resolve được ủy quyền thành công.
...
- **Then** AI Assistant kích hoạt toàn bộ công cụ của học viên (`query_notices`, `search_web`) kết hợp với bộ công cụ quản trị nâng cao:
  1. `evaluate_radar` & `resolve_question`: Quét và đánh dấu giải quyết các ticket quá hạn với khóa lạc quan (optimistic locking).
  2. `create_staff_alert`: Tạo cảnh báo nội bộ, không gửi DM hoặc công bố ra kênh học viên.
  3. `format_daily_digest`: Xuất bản tin 22:00 sạch lỗi.
- **And** Các công cụ `broadcast_notification`, `check_student_profile` và `check_scores` không được khai báo cho agent vì vượt phạm vi thông báo đã xác minh và quyền riêng tư,
